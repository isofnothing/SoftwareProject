...
<!--            {{ findRoleName(scope.row.role_id) }}-->
            <span :class="getRoleColorClass(scope.row.role_id)">{{ findRoleName(scope.row.role_id) }}</span>
          </template>
        </el-table-column>
...
const getRoleColorClass = (roleId) => {
  return roleColors.value[roleId] || 'role-default';
};
...
.role-default {
  ...
  padding: 6px 12px; /* 增加内边距以模仿边框留下的空间 */
